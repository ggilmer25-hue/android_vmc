...
class VMCAndroidApp:
    def __init__(self, page: ft.Page):
        ...
        # `page.window` may not exist on some platforms/runtimes — guardarlo
        ...
        if win:
            try:
                ...
                # Si la API no lo permite, ignoramos y seguimos
                pass
        self.page.bgcolor = ft.colors.BACKGROUND

        # Obtener directorio base correctamente
        ...
        self.save_fab = ft.FloatingActionButton(
            icon=ft.Icon(ft.icons.SAVE, color="white"),
    ...
    def cargar_datos_excel_habilidades(self):
        """Lee el Excel usando openpyxl (sin pandas)"""
        if not os.path.exists(self.archivo_excel):
            print(f"Archivo no encontrado: {self.archivo_excel}")
            return
            
        try:
            wb = load_workbook(self.archivo_excel, data_only=True)
            ws = wb.active
            ...
            # Filtrar nombres no deseados
            datos = [p for p in datos if 'Nombre' in p and not any(x in str(p['Nombre']) for x in ['Spolzino', 'Saucedo'])]
            ...
            for p in datos:
                for col in columnas_hab + ['Es_Menor', 'Telefono', 'Privilegio', 'Genero']:
                    if col not in p:
                        p[col] = 'No' if col in columnas_hab + ['Es_Menor'] else ''
                p['Telefono'] = str(p.get('Telefono', '')).replace('.0', '').strip()
            
            # Fusionar o usar el Excel como roster principal
            if not self.congregacion_data:
                self.congregacion_data = datos
            else:
                excel_map = {row['Nombre']: row for row in datos if 'Nombre' in row}
                for p in self.congregacion_data:
                    n = p.get('Nombre')
                    if n and n in excel_map:
    ...
    def recargar(self, e=None):
        self.cargar_datos()
        idx = self.nav_bar.selected_index if hasattr(self, 'nav_bar') else 0
        if idx == 0:
            ...
            content=ft.Row(
                controls=[
                    ...
                    ft.IconButton(
                        ...
                        icon_color="#FFD54F",
                        tooltip="Generar Bimestre de Prueba",
    ...
    # ==========================================
    # MODAL DE GENERACIÓN (VERSIÓN SIMPLIFICADA)
    # ==========================================
    def abrir_dialogo_generar(self, e):
        ...
        dialog_content = ft.Column([
            ft.Text("Genera asignaciones de prueba para el bimestre seleccionado.", size=13),
            ft.Divider(height=10),
            year_dropdown,
            bimestre_dropdown,
            ft.Text("Esto sobrescribirá el programa actual.", size=11, color="red")
        ], spacing=10, height=200, width=320)

        def on_generar_click(ev):
            self.page.dialog.open = False
            self.page.update()
            self._ejecutar_generacion_prueba(year_dropdown.value, bimestre_dropdown.value)

        self.page.dialog = ft.AlertDialog(
            title=ft.Text("🔄 Generar Bimestre de Prueba"),
            content=dialog_content,
            actions=[
                ft.TextButton("Cancelar", on_click=lambda _: setattr(self.page.dialog, "open", False) or self.page.update()),
                ft.ElevatedButton("Generar", bgcolor="#4A148C", color="white", on_click=on_generar_click)
            ],
    ...
    def _ejecutar_generacion_prueba(self, año, bimestre_nombre):
        """Genera un bimestre de prueba sin necesidad de scraper"""
        
        # Mostrar diálogo de progreso
        progreso_ring = ft.ProgressRing()
        progreso_text = ft.Text("Generando asignaciones...", size=14)
        ...
        def background_task():
            try:
                ...
                meses = meses_map.get(bimestre_nombre, ["Enero", "Febrero"])
                
                # Crear 8 semanas de prueba (típicamente un bimestre)
                for i in range(8):
                    progreso_text.value = f"Generando semana {i+1}/8..."
                    self.page.update()
                    
                    # Fechas simuladas
                    semana_fecha = f"Semana {i+1} de {meses[0]} - {int(año)}"
                    if i >= 4:
                        semana_fecha = f"Semana {i-3} de {meses[1]} - {int(año)}"
                    
                    # Partes de maestros simuladas
                    maestros_parts = [
                        "Conversación (Enc./Ayu.)",
                        "Revisita (Enc./Ayu.)",
                        "Discipulado (Enc./Ayu.)",
                        "Discurso"
                    ]
                    
                    # Inicializar asignador con los datos actuales
                    if not self.congregacion_data:
                        self.show_message("❌ No hay datos de congregación para generar asignaciones.", is_error=True)
                        self.page.dialog.open = False
                        self.page.update()
                        return
                    
                    asignador = AsignadorVMCMovil(self.congregacion_data, {})
                    
                    res_semana = asignador.generar_semana(
                        index_semana=i+1,
                        fecha=semana_fecha,
                        lectura=f"Lectura de la semana {i+1}",
                        asigs_maestros=maestros_parts
                    )
                    semanas_generadas.append(res_semana)
                
                # Guardar semanas generadas
                self.bimestre_data = semanas_generadas
                
                # Guardar el historial de maestros
                ...
                self.page.dialog.open = False
                self.page.update()
                self.show_message("🎉 ¡Bimestre generado con éxito!")
                self.recargar()
    ...
    def _mostrar_bimestre(self):
            ...
            self.content_area.controls.append(
                ft.Container(
                    content=ft.Column([
                        ...
                        ft.Text("Pulsa el botón de varita mágica para generar un bimestre de prueba o importa un JSON.",
        ...
        for sem_idx, semana in enumerate(self.bimestre_data):
            if semana.get('no_reunion', False):
                ...
                week_title_bg = "#4A148C" if self.page.theme_mode != ft.ThemeMode.DARK else "#311B92"
    ...
    def _construir_subfila_tactual(self, semana_idx, rol, valor, campo_json, skill_col, sub_idx=None):
        name_color = "#3949AB" if self.page.theme_mode != ft.ThemeMode.DARK else "#8C9EFF"
            ...
            if tel:
                action_buttons.append(
                    ft.IconButton(
                        ...
                        on_click=lambda _, v=valor, t=tel, r=rol: self._enviar_whatsapp_individual(v, t, r, semana_idx)
                    )
                )
    ...
    def abrir_selector_candidato(self, semana_idx, rol, valor_actual, campo_json, skill_col, sub_idx=None):
        ...
        def build_tiles(filter_text=""):
            ...
            for item in sugerencias:
                ...
                color_bg = "#FFE0B2" if item['Ocupado'] else "#E8F5E9" if item['Privilegio'] == 'Anciano' else "white" if self.page.theme_mode != ft.ThemeMode.DARK else ft.colors.SURFACE_VARIANT
            ...
            for item in todos:
                ...
                badge_occup = " • [Ocupado]" if item['Ocupado'] else ""
                color_bg = "#FFE0B2" if item['Ocupado'] else "white" if self.page.theme_mode != ft.ThemeMode.DARK else ft.colors.SURFACE
        ...
        selector_container = ft.Container(
            content=ft.Column([
                ft.Row([
                    ...
                    ft.IconButton(ft.icons.CLOSE, on_click=lambda _: setattr(self.page.bottom_sheet, "open", False) or self.page.update())
                ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
    ...
    def _mostrar_congregacion(self):
        ...
        def show_publisher_profile(p):
            ...
            if habilidades_si:
                ...
                habs_chips.append(ft.Text("Sin habilidades asignadas", size=11, italic=True))
            ...
            dialog_profile = ft.AlertDialog(
                ...
                actions=[
                    ft.TextButton("Cerrar", on_click=lambda _: setattr(self.page.dialog, "open", False) or self.page.update())
                ]
            )
            self.page.dialog = dialog_profile
        ...
        def filter_roster(query=""):
            ...
            for row in datos_ordenados:
                ...
                if priv != prev_priv and not q:
                    color_bg = "#EDE7F6" if self.page.theme_mode != ft.ThemeMode.DARK else "#311B92"
                    text_color = "#4A148C" if self.page.theme_mode != ft.ThemeMode.DARK else "#E0D7FF"
    ...
    # ==========================================
    # PESTAÑA 3: NOTIFICACIONES WHATSAPP
    # ==========================================
    def _mostrar_notificaciones(self):
        ...
        for nombre_hermano, partes in sorted(agenda_personal.items()):
            ...
            for p in partes:
                partes_texto.append(f"• *{p['rol']}* — 📅 *{p['fecha']}*")
                list_widgets.append(
                    ft.Container(
                        content=ft.Row([
                            ...
                            ft.Column([
                                ...
                                ft.Text(f"📅 {p['fecha']}", size=11, color="#757575")
            ...
            if telefono:
                ...
                def click_envio(e, name=nombre_hermano, tel=telefono, parts_str=partes_str):
                    saludos = [
                        f"Hola {name}, 🙂",
                ...
                action_buttons.append(
                    ft.Text("⚠️ Sin teléfono registrado", color="red", size=12, weight="bold")
                )
...
def main(page: ft.Page):
    try:
        ...
        page.add(
            ft.Text("❌ ERROR CRÍTICO AL INICIAR:", color="red", weight="bold", size=18),
